...
class Location_finder(object):
    def __init__(self, clVers, **args):
        ...
        for j in t:
            ...
            jmfit.blc[1] = .80*self.args["cleanBox"][1]#fraction to increase box size
            jmfit.blc[2] = .80*self.args["cleanBox"][2]
            jmfit.trc[1] = 1.20*self.args["cleanBox"][3]
            jmfit.trc[2] = 1.20*self.args["cleanBox"][4]
            jmfit.niter = 1000
            ...
            DEC = [None]*4
            address2 = os.getcwd() + '/images_{0}/'.format(self.args["date"]) + imgname + '.coord' 
            print address2
            with open(address2, 'a') as f:
                for l in f.readlines():
                    temp = l.split()
